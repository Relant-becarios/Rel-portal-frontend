...
const estadoPendientes = computed(() => {
  ...
  if (pendientes.length === 0) {
    return { cantidad: 0, mensaje: 'Hoy no tienes pendientes operacionales.', detalle: '¡Excelente! Tienes la mesa limpia.' }
  }
  ...
  if (!fechaCreacion) {
    return { cantidad: pendientes.length, mensaje: `Tienes ${pendientes.length} pendientes activos.`, detalle: 'Esperando atención.' }
  }
  ...
  let tiempoTexto = diffDias > 0 ? `${diffDias} día(s)` : diffHoras > 0 ? `${diffHoras} hora(s)` : diffMins > 0 ? `${diffMins} minuto(s)` : 'unos instantes'

  return {
    cantidad: pendientes.length,
    mensaje: `Tienes tareas acumuladas desde hace ${tiempoTexto}.`,
    detalle: `Actualmente hay ${pendientes.length} requerimiento(s) activo(s).`
  }
})
</script>
...
<template>
  <div :class="esModoOscuro ? 'bg-zinc-950 text-zinc-100' : 'bg-slate-50 text-slate-800'" class="flex min-h-screen transition-colors duration-200 relative overflow-x-hidden font-sans">
    ...
    <div class="flex-1 flex flex-col min-w-0 w-full relative">
      ...
      <main class="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col items-center justify-start space-y-4 pt-2 sm:pt-4 pb-24">
        
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-5xl mx-auto">
          <div :class="esModoOscuro ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200 shadow-sm'" class="p-4 sm:p-5 rounded-2xl border flex items-center justify-between text-left">
            <div>
          ...
          <div :class="esModoOscuro ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200 shadow-sm'" class="p-4 rounded-2xl border flex items-center justify-between text-left">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 w-full max-w-5xl">
          <div :class="esModoOscuro ? 'from-red-950/30 to-zinc-900 border-zinc-800' : 'from-red-50/40 to-white border-slate-200 shadow-sm'" class="bg-linear-to-b border lg:col-span-6 p-4 sm:p-6 rounded-2xl flex flex-col justify-between space-y-4">
            <div class="space-y-3">
              <div class="inline-flex p-2 bg-red-700/10 text-red-500 rounded-xl border border-red-500/20 text-2xl">👋</div>
          ...
          <div :class="esModoOscuro ? 'from-zinc-900 to-zinc-950 border-zinc-800' : 'from-slate-100/50 to-white border-slate-200 shadow-sm'" class="bg-linear-to-b border lg:col-span-6 p-4 sm:p-6 rounded-2xl flex flex-col justify-between space-y-4">
            <div class="space-y-3 text-left">
              <span class="text-[9px] font-bold text-amber-500 uppercase tracking-widest block font-sans">Mesa de Control Central</span>
              <h3 class="text-base sm:text-lg font-black" :class="esModoOscuro ? 'text-white' : 'text-slate-900'">Requerimientos Activos</h3>
      ...
      <!-- ☀️/🌙 BOTÓN FLOTANTE INFERIOR DERECHO (BOTTOM-RIGHT) -->
      <button 
        @click="toggleTema" 
